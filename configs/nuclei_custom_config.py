...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file='./coco_annotations_all.json',
        img_prefix='../../DATA/Nuclei/dataset/all/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
            dict(type='Resize', img_scale=(800, 800), keep_ratio=True),
    ...
    test=dict(
        ...
        img_prefix='../../DATA/Nuclei/dataset/test/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
...
log_level = 'INFO'
load_from = './work_dirs/fin_resnet_rcnn_s4_800_noCrop/epoch_24.pth'
resume_from = None
workflow = [('train', 1)]
